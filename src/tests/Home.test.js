...
test('Render exercise Track button +', () => {
  const { getByText } = render(componentWrap(<Home />));
  const headerText = getByText(/13/);
  expect(headerText).toBeInTheDocument();
});

test('Render exercise name', () => {
  const { getByText } = render(componentWrap(<Home />));
  const headerText = getByText(/KICK BACK/);
  expect(headerText).toBeInTheDocument();
});

test('Render exercise label', () => {
  const { getAllByText } = render(componentWrap(<Home />));
  const headerText = getAllByText(/latest/);
  expect(headerText[0]).toBeInTheDocument();
});

test('Render exercise count', () => {
  const { getByText } = render(componentWrap(<Home />));
  const headerText = getByText(/13/);
  expect(headerText).toBeInTheDocument();
});

test('Renders exercise image', () => {
  const { getAllByAltText } = render(componentWrap(<Home />));
  const headerText = getAllByAltText(/kick back/i);
  expect(headerText[0]).toBeInTheDocument();
});
